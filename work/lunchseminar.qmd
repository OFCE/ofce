...
-   pour appliquer à un `qmd` existant\
    `setup_quarto()` (ne fait que remettre `_extensions`)

Il est possible de spécifier des paramètres dans le `yaml` en plus ou de modifier les codes `html`, `latex`, `scss`.
...
cette fonction sert à appliquer le Système International pour produire une chaîne de caractère compacte et lisible. Pratique pour les messages ou les échelles.

```{r}
ofce::f2si2(c(142010, 2000000, 0.1928), digits = 2) 
```

## Evolutivité du package
...
-   package encore en évolution : signalez vos difficultés, les bugs, vos souhaits
    ...
    -   modèle de documents supplémentaires (policy brief par exemple)
