...
```
  ...
  - On les trouve :
    ...
    + via des canaux privés : le package est transmis directement au format `.tar.gz` ou `.zip`. 
    Attention à la source, peu de garanties sur leur utilisation
    
    
## Le package `{pak}` : une alternative pour les gens cool 
...
###  3. Installation du package `{ofce}` {.tabset}
