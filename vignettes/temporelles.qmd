...
Prenons l'exemple du graphiques sur les spreads ([legislatives2024, Blot Gerrolf Plane](https://www.ofce-legislatives2024.fr/analyses/spreads.html#fig-spread)). Les données sont générées par un scrapping sur *investing.com* (en attendant une solution API sur une banque de données bien faite). Les données se présentent sous la forme.
...
add_logo(main %+% ss) +
  ggrepel::geom_text_repel(aes(label=label_pays, color = pays), 
                           size = 3, show.legend=FALSE, 
                           segment.size = 0.2, min.segment.length = 2,
                           max.overlaps = Inf, hjust= 0.5) +
  ggrepel::geom_text_repel(aes(label=event), color = "black", 
                           size = 2, show.legend=FALSE, 
                           segment.size = 0.2, min.segment.length = 0.1,
                           max.overlaps = Inf, hjust= 0.5,nudge_x = c(-500, -250, -250), nudge_y = c(-0.3,1, -0.5),
...
inset <- main  + 
  ...
  geom_vline(xintercept = as.Date("2024-06-09"),
             linewidth = 0.1,
             color = "grey50") +
  geom_vline(xintercept = as.Date("2024-07-08"),
             linewidth = 0.1,
             color = "grey50") +
...
Le package `{ggmagnify}` simplifie la tâche et offre quelques améliorations esthétiques. Il faut cependant que les dataset principal et inset soit les mêmes. On reprend l'agrégation temporelle en l'arrêtant aux deux derniers mois.
On peut facilement complexifier l'insert pour intégrer plus d'éléments en utilisant l'argument `plot` de `ggmagnify::geom_magnify()`.
...
spreads_hyb <- spreads_data |> 
  ...
  mutate(
    date_label = ifelse(n>1,
                        stamp("juillet 2024", orders = "%B %Y",  locale = "fr_FR.utf8", quiet = TRUE)(date),
                        stamp("22/12/2024", orders = "%d/%m/%Y",  locale = "fr_FR.utf8", quiet = TRUE)(date)),
    tooltip = str_c(
        "<b>", pays, "</b><br>",
        date_label,
...
inset_plot <- ggplot(spreads_hyb) +
  ...
  theme_ofce(base_size = 7,
             ...
             axis.line.y = element_blank()) +
  geom_vline(xintercept = as.Date("2024-7-7"))+ 
  labs(
    y=NULL,
    x=NULL,
    colour=NULL,
...
sh <- ggplot(spreads_hyb) +
  ...
  scale_x_date(labels = scales::label_date_short(format = c("%Y")),
               ...
               date_minor_breaks = "1 year",
               guide = "minor_ticks") +
  scale_y_continuous(labels = ~str_c(.x, "%"), breaks=seq(0,5)) +
  annotations +
