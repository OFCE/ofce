...
1.  On utilise `theme_ofce()`

2.  Les dates sont au format `<date>` même lorsque la fréquence est annuelle

3.  On utilise `scale_x_date(date_breaks = "5 years", date_minor_breaks = "1 year", guide = "minor_ticks")` en définissant `date_breaks` à la fréquence souhaitée (en évitant trop de dates) et `date_minor_breaks` à une année (`"1 year"`).

4.  Si les *y* sont en %, alors mettre "%" dans le format de l'axe des *y*

5.  On choisit un `line_width` entre 0.5 et 1 pour le `geom_line()`, on ajoute un `geom_point(shape=21, stroke=0.25, col="white")` pour marquer les points auxquels on a des données. Si on peut mettre la légende sous forme de *labels*, c'est mieux car cela allège le graphique.
...
spreads_q <- spreads_data |> 
  mutate(date_q = lubridate::floor_date(date, unit="quarter")) |> 
  group_by(date_q, pays) |> 
  summarize(taux_max = max(taux, na.rm=TRUE),
            taux_min = min(taux, na.rm=TRUE),
            taux = mean(taux, na.rm=TRUE)) |> 
  rename(date = date_q)
...
ggplot(spreads_q) +
  ...
  labs(
    ...
    colour=NULL,
    caption = glue::glue("*Source* : investing.com<br>Mis à jour : {date_maj}")) + 
  scale_x_date(labels = scales::label_date_short(format = c("%y", "%Q")),
               date_breaks = "1 year", 
               date_minor_breaks = "1 year",
               guide = "minor_ticks") +
  scale_y_continuous(labels = ~str_c(.x, "%"))
