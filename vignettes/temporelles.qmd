...
ggplot(spreads_q) +
  aes(x=q, y=taux, fill = pays, color=pays, group=pays) +
  geom_line(linewidth = 0.75, alpha = 0.5, show.legend = FALSE) +
  geom_point(stroke = 0.5, size = 1, 
             col = "white", shape = 21, show.legend = FALSE)+
  scale_color_manual(aesthetics = c("fill", "color"), 
  ...
  scale_y_continuous(labels = ~str_c(.x, "%")) +
    facet_wrap(vars(y), nrow = 1, strip.position = "bottom") +
  theme(strip.placement = "outside",
        strip.text = element_text(margin = margin()),
        axis.text.x = element_text(margin = margin(t=3, b=-2))) + 
  scale_y_continuous(labels = ~str_c(.x, "%"))+
  scale_x_discrete( )
...
ggplot(spreads_q) +
  ...
  scale_x_date(
    ...
    guide = "minor_ticks", limits = c(as.Date("2020-01-01"), NA)) +
  theme(axis.text.x = element_text(hjust=0))+
  scale_y_continuous(labels = ~str_c(.x, "%")) +
