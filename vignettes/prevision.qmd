...
data_gpib <- read_excel("data_pays.xlsx", sheet = "pib", col_names = TRUE) |>
...
data_tcho <- read_excel("data_pays.xlsx", sheet = "tcho", col_names = TRUE) |>
  mutate(date = as.Date(date)+ days(45)) |>
  pivot_longer(cols=-date, names_to="pays", values_to = "value") |>
  mutate(
    variable = "Chômage",
...
data_prix <- read_excel("data_pays.xlsx", sheet = "prix", col_names = TRUE) |>
  mutate(date = as.Date(date) + days(45)) %>%
...
data_solde <- read_excel("data_pays.xlsx", sheet = "solde", col_names = TRUE) |>
