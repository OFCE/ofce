...
Lorsqu'on édite un `.qmd` avec RStudio on dispose de deux modes. Le premier, *Source*, est l'édition d'un fichier en mode texte. On y utilise la syntaxe *markdown*. L'avantage est que quand il y a des petits bugs dans le texte on peut les repérer très vite. L'aternative est le mode *Visual* qui affiche le texte formaté et ajoute une barre de menu pratique pour le formattage ou l'insertion d'éléments.
