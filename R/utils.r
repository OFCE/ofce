...
graph2svg <- function(graph, file="", rep="svg", ratio = 4/3, height = width/ratio, width = 18, units="cm", bg="white",...)
{
  ...
  cl <- dplyr::case_when(
    ...
    TRUE ~ "err")
  switch(cl,
         gg =  ggplot2::ggsave(filename=fn, plot=graph, height = height, width = width, units=units, bg=bg, ...),
...
graph2jpg <- function(graph, file="", rep="svg", ratio = 4/3, height = width/ratio, width = 18, units="cm", bg="white", quality = 100, ...)
{
  ...
  cl <- dplyr::case_when(
    ...
    TRUE ~ "err")
  switch(cl,
         gg = ggplot2::ggsave(filename=fn, plot=graph, height = height, width = width, units=units, quality=quality, bg=bg, ...),
